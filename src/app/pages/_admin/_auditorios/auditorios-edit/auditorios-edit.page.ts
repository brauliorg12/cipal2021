...
@Component({
  ...
  templateUrl: './auditorios-edit.page.html',
  styleUrls: ['./auditorios-edit.page.scss'],
})
export class AuditoriosEditPage implements OnInit {
  ...
  updateOfferNoImage() {
    this.presentLoading('Modificando datos...').then((value) => {
      ...
      this.productoService.updateExpo(this.offerForm.value, this.offerForm.value).subscribe(async itemsLoad => {
        if (itemsLoad) {
